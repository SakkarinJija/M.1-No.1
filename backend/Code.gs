...
function upgradeV4(){const ss=getDb_();backfillLogIds_(ss.getSheetByName('Logs'));PropertiesService.getScriptProperties().setProperty('V4_MIGRATED','1');Logger.log('Version 4 ready: '+ss.getUrl());}

function registerStudent_(p){
  verify_(p.classCode);if(!p.studentId||!p.name||!p.className)throw Error('กรอกข้อมูลให้ครบ');
...
function updateLog_(p){
  verify_(p.classCode);validateLog_(p);if(!p.logId)throw Error('ไม่พบรหัสรายการ');
  const ss=getDb_(),sh=ss.getSheetByName('Logs'),row=objects_(sh).find(r=>r.LogId===p.logId&&r.StudentKey===p.studentKey);
  if(!row)throw Error('ไม่พบบันทึกที่ต้องการแก้ไข');
...
function deleteLog_(p){
  verify_(p.classCode);if(!p.studentKey||!p.logId)throw Error('ข้อมูลไม่ครบ');
  const sh=getDb_().getSheetByName('Logs'),row=objects_(sh).find(r=>r.LogId===p.logId&&r.StudentKey===p.studentKey);
  if(!row)throw Error('ไม่พบบันทึกที่ต้องการลบ');sh.deleteRow(row._row);return{ok:true};
}

function validateLog_(p){if(!p.studentKey||!p.date||!p.level||!p.source||!p.activity)throw Error('กรอกข้อมูลกิจกรรมให้ครบ');}

function studentData_(p){
  verify_(p.classCode);const ss=getDb_(),st=objects_(ss.getSheetByName('Students')).find(r=>r.StudentKey===p.studentKey);if(!st)throw Error('ไม่พบนักเรียน');
  const logs=objects_(ss.getSheetByName('Logs')).filter(r=>r.StudentKey===p.studentKey).slice(-500);
  return{ok:true,version:4,profile:profile_(st),logs:logs.map(clean_),classInfo:getClassInfo_(ss)};
}
...
function teacherData_(p){
  ...
  return{ok:true,version:4,generatedAt:new Date().toISOString(),students:out,weekly:weekly_(logs,12),sourceUsage:src,classInfo:getClassInfo_(ss)};
}

function studentDetail_(p){
  teacherVerify_(p);const ss=getDb_(),st=objects_(ss.getSheetByName('Students')).find(r=>r.StudentKey===p.studentKey);if(!st)throw Error('ไม่พบนักเรียน');
...
function saveTeacherNote_(p){
  teacherVerify_(p);if(!p.studentKey)throw Error('ไม่พบนักเรียน');
...
function teacherVerify_(p){if(String(p.pin)!==String(TEACHER_PIN))throw Error('รหัสครูไม่ถูกต้อง');verify_(p.classCode);}
...
function headerIndex_(sh,name){const h=sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0],i=h.indexOf(name);if(i<0)throw Error('ไม่พบคอลัมน์ '+name);return i+1;}
